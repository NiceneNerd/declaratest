...
fn convert_style(rust_style: &docx_rust::styles::Style) -> Option<docx_rs::Style> {
    ...
    if let Some(ref char_props) = rust_style.character {
        ...
        if let Some(ref sz) = char_props.size {
            let size = sz.value.unsigned_abs(); // Convert isize to usize
                ...
                Ok(docx) => {
                    ...
                    for style in &docx.styles.styles {
                        if let Some(converted_style) = convert_style(style) {
                            ...
                            template_info
                                .parsed_styles
                                .insert(style_id, converted_style);
                            converted_count += 1;
                        }
                    }

                    println!(
                        "✓ Successfully converted {} styles from template",
                        converted_count
                    );
                    ...
                    if has_title {
                        println!("  - Title style found");
                    }
                    if has_subtitle {
                        println!("  - Subtitle style found");
                    }
                    if has_heading1 || has_heading2 {
                        println!("  - Heading styles found");
                    }
                    if has_list_number {
                        println!("  - List Number style found");
                    }
                }
                Err(e) => {
